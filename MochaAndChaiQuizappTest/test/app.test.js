...
except(result.message).to.equal("All fields are required");

        } );
        it('should fail if passwords do not match', function(){
            const result = register({
                firstName: "john",lastName :"Doe",email :"invlaid-email",contact : "1234567890",
                password: "password123", confirmPassword:"password123",role:"Student",branch: "CDAC",gender: "Male"
                });
expect(result.success).to.be.false;
except(result.message).to.equal("Passwords do not match");
        } );



        it('Should fail if passwords is less than 6 characters', function() {
            const result = register({ firstName : 'jOhn', lastName:'Doe', email:"Test@example.com", contact:"1234567890", 
        ...
        it('Should fail if email format is invaild', function() {
            const result = register({ firstName : 'jOhn', lastName:'Doe', email:"invali-email", contact:"1234567890", 
        ...
        it('Should fail if contact number is ot 10 digit', function() {
            const result = register({ firstName : 'jOhn', lastName:'Doe', email:"john@gmail.com", contact:"1234560", 
                password:"password123", confirmPassword:"password123", role:"Student", branch:"CDAC", gender: "male"
            })
            expect(result.success).to.be.false;
        ...
        it('Should success if all fields are valid', function() {
            const result = register({ firstName : 'John', lastName:'Doe', email:"john@gmail.com", contact:"1234567890", 
                password:"password123", confirmPassword:"password123", role:"Student", branch:"CDAC", gender: "male"
            })
